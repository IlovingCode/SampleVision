...
type ArStatelessEvent = SyntheticEvent<{}, { detected: string }>;

type ArViewerProps = {
  model: string;
  planeOrientation?: 'any' | 'vertical' | 'horizontal' | 'none' | 'image';
  allowScale?: boolean;
  allowRotate?: boolean;
...
> {
  ...
  /**
   * Set type of detection
   * @returns void
   */
  setPlaneOrientation(detection: string) {
    this.nativeRef.current &&
      UIManager.dispatchViewManagerCommand(
        findNodeHandle(this.nativeRef.current as unknown as number),
        (UIManager as ArViewUIManager)[ComponentName].Commands.setPlaneOrientation,
        [detection]
      );
  }
   ...
   * @returns void
   */
  loadArImage(path: string[]) {
    this.nativeRef.current &&
      UIManager.dispatchViewManagerCommand(
        findNodeHandle(this.nativeRef.current as unknown as number),
        (UIManager as ArViewUIManager)[ComponentName].Commands.loadArImage,
        [path]
      );
  }
