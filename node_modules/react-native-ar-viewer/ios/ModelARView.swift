...
//                self.onModelPlacedHandler([:])
    ...
    func setUp() {
        ...
        arImages = ARReferenceImage.referenceImages(inGroupNamed: "AR Resources", bundle: nil)
...
//        }
//
//        grid.isEnabled = true4
//        grid.replaceModel(model: modelEntity)
        onModelPlacedHandler(["detected":grid.name])
        isModelVisible = true
//        setGestures()
    }
    ...
    func start() {
        if (isSetup && readyToStart) {
            self.session.run(self.config, options: [.resetTracking, .removeExistingAnchors, .stopTrackedRaycasts, .resetSceneReconstruction])
            if(!sessionStarted) {
                sessionStarted = true
    ...
    func getData(from url: String) async -> UIImage {
...
//        print("Download Started")
        Task {
            for url in urls {
                let image = await getData(from: url)
                arImages?.insert(ARReferenceImage(image.cgImage!, orientation: CGImagePropertyOrientation.up, physicalWidth: 3))
            }
        }
    ...
    func session(_ session: ARSession, didAdd anchors: [ARAnchor]) {
        for anchor in anchors {
            if let _ = anchor as? AREnvironmentProbeAnchor { return }
    ...
    func session(_ session: ARSession, didUpdate anchors: [ARAnchor]) {
        for anchor in anchors {
            let updatedGrids: [Grid]? = grids.filter { grd in
                grd.myAnchor.identifier == anchor.identifier }
            for grid in updatedGrids ?? [] {
                if (!grid.isShowingModel && grid.isEnabled) {
                    grid.transform.matrix = anchor.transform
                }
                grid.didUpdate(anchor: anchor)
            }
        }
    }
